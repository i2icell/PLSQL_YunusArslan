CREATE OR REPLACE PROCEDURE CHECK_USER(PIS_PHONE IN VARCHAR2, PIS_PASS IN VARCHAR2, CHECKER OUT NUMBER) IS
V_COUNT NUMBER :=0;
BEGIN
...
CREATE OR REPLACE PROCEDURE GET_BALANCES(PIS_PHONE IN VARCHAR2, REMAIN_GB OUT NUMBER, REMAIN_MIN OUT NUMBER, REMAIN_SMS OUT NUMBER) IS
...
BEGIN
    ...
    IF REMAIN_GB>0 OR REMAIN_MIN>0 OR REMAIN_SMS>0
    THEN SELECT ROUND(DBMS_RANDOM.VALUE(1,10)) RANDOM INTO RANDOM_NUMBER FROM DUAL;
            REMAIN_GB   :=  REMAIN_GB-RANDOM_NUMBER/100;
            REMAIN_MIN  :=  REMAIN_MIN-2*RANDOM_NUMBER;
            REMAIN_SMS  :=  REMAIN_SMS-RANDOM_NUMBER-5;
...
BEGIN
    ...
    VALUES (SEQ_USER_ID.NEXTVAL,UPPER(PIS_F_NAME),UPPER(PIS_L_NAME),PIS_P_NUMBER,PIS_E_MAIL,MD5_HASH(PIS_P_WORD),TO_DATE(PIS_B_DAY,'DD/MM/YYYY'),PIS_TC_NUMBER);
...
CREATE OR REPLACE PROCEDURE CHANGE_PASSWORD(PIS_PHONE IN VARCHAR2, PIS_TC_NUMBER IN VARCHAR2, PIS_NEW_PASS IN VARCHAR2, CHECKER OUT NUMBER) IS
CHECK_TC_PHONE NUMBER;
BEGIN
    SELECT COUNT(*)INTO CHECK_TC_PHONE FROM USER_INFO WHERE PHONE_NUMBER=PIS_PHONE AND TC_NO=PIS_TC_NUMBER AND PASS_WORD<>MD5_HASH(PIS_NEW_PASS);
...
DECLARE
RANDOM_NUMBER NUMBER(5,2);
BEGIN
SELECT ROUND(DBMS_RANDOM.VALUE(1,100)) RANDOM INTO RANDOM_NUMBER FROM DUAL;
