...
BEGIN
    ...
    IF CHECK_PACK=1 THEN 
        ...
        UPDATE REMAIN_BALANCE   SET GB_REM=REMAIN_GB,
                                ...
                                WHERE PHONE_NUMBER=PIS_PHONE;
        CHECKER:=1;
        DBMS_OUTPUT.PUT_LINE('Yeni paketin hesabýna tanýmlandý.');
        COMMIT;
        ELSE
        CHECKER:=0;
        DBMS_OUTPUT.PUT_LINE('Paket tanýmlanamadý. Ýstenilen paket mevcut deðil.');
        END IF;
END BUY_NEW_PACKAGE;
/
...
BEGIN
    ...
    SELECT PACKAGE_NAME FROM PACK_DEF;
    DBMS_OUTPUT.PUT_LINE('i2iCell güncel paketleri þöyledir:');
    LOOP
        FETCH POS_PACKAGES INTO X; 
        ...
        dbms_output.put_line(X||': '||SHOW_GB||' GB'||' '||SHOW_MIN||' Minutes'||' '||SHOW_SMS||' SMS');
    END LOOP;
END LIST_PACKAGES;
/
